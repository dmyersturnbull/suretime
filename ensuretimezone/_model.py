...
class TaggedDuration:
    ...
    @property
    def n_nanos(self) -> int:
        return self.end.mono_clock_ns - self.start.mono_clock_ns

    @property
    def n_micros(self) -> int:
        return int(round((self.end.mono_clock_ns - self.start.mono_clock_ns) / 1000))

    @property
    def delta(self) -> timedelta:
        return timedelta(microseconds=self.n_micros)

    @property
    def exact_duration_str(self) -> str:
        micros = self.n_micros
        return str(timedelta(microseconds=micros)) + "." + str(self.n_nanos % 1000)

    def __str__(self) -> str:
        return f"{self.start.iso_with_zone} to {self.end.iso_with_zone} ({self.n_nanos}"


__all__ = ["TaggedDatetime", "TaggedDuration", "TzMapType", "TzDictType"]
